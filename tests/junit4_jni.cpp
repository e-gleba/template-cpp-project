...
Java_com_egleba_app_AppActivityTest_getTestNames(JNIEnv *env, const jclass) {
    ...
    if (!java_string_class) {
        ...
        throw std::runtime_error(
                "java class not found: "s + jstring_class_name);
    }
    ...
    if (!tests_to_run) {
        throw std::runtime_error("failed to allocate java object array of length " +
                                 std::to_string(tests.size()));
    }

    jsize i = 0;
    for (auto &tc: tests) {
        env->SetObjectArrayElement(tests_to_run, i,
                                   env->NewStringUTF(tc.data()));
        env->ExceptionDescribe();
        i++;
    }
    return tests_to_run;
